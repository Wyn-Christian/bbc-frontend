...
export function createCustomShadows(mode) {
  const colorChannel = mode == "light" ? grey["500Channel"] : "0 0 0";
  ...
  return {
    ...
    z24: `0 24px 48px -4px ${varAlpha(colorChannel, 0.16)}`,
    //
    dialog: `-40px 40px 80px -8px ${varAlpha(common.blackChannel, 0.24)}`,
    card: `0 0 2px -0 ${varAlpha(colorChannel, 0.2)}, 
						0 12px 24px -4px ${varAlpha(colorChannel, 0.12)}`,
    dropdown: `0 0 2px 0 ${varAlpha(colorChannel, 0.24)}, 
